...
public class TVShowPopularVM extends ViewModel {
    private Api api;
    private static final String TAG = MainActivity.class.getSimpleName();
    private MutableLiveData<ArrayList<TVShow>> data = new MutableLiveData<>();

    public void setTvPopular(String language){
        if (this.api == null){
            api = new Api();
        }
        api.getUrl().getTVShowPopular(language).enqueue(new Callback<TVShowResponse>() {
